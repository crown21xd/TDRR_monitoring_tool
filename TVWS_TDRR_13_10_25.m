...
for trial_index=1:num_trials
  ...
  for snr_index = 1:length(snr_dB_list)
    ...
    for ofdm_symbol_index=1:num_ofdm_symbols
      ...
      jammer_signal_freq = sqrt(0.5)*randn(1,num_subcarriers)+ 1j*sqrt(0.5)*randn(1,num_subcarriers);  % w_f
      jammer_signal_time_no_cp = sqrt(num_subcarriers)*ifft(jammer_signal_freq); % w_no_cp
      jammer_signal_time = [jammer_signal_time_no_cp(:,num_subcarriers-cyclic_prefix_length+1:num_subcarriers), jammer_signal_time_no_cp];  % w
      jammer_signal_time = sqrt(jammer_power_linear)*jammer_signal_time;  % scale jammer power
  
      % generate jammer's transmit signal in time domain (OFDM-noncompliant)
      jammer_signal_time_noncomp = sqrt(0.5)*randn(1,num_subcarriers+cyclic_prefix_length)+ 1j*sqrt(0.5)*randn(1,num_subcarriers+cyclic_prefix_length); % w_noncomp
      jammer_signal_time_noncomp = sqrt(jammer_power_linear)*jammer_signal_time_noncomp;  % scale jammer power       
      ...
      % generate UEs' transmit signal in frequency domain (OFDM-compliant)
       for tone_index=1:num_used_subcarriers
            indices(trial_index,ofdm_symbol_index,:,tone_index) = bi2de(bits(trial_index,ofdm_symbol_index,:,:,tone_index),'left-msb')+1; % idx
       end
    ...
    for tone_index=1:num_used_subcarriers
      ...
      if num_receive_antennas == 1
          projection_matrix_compliant = eye(num_receive_antennas);  % no nulling in SISO
      end
      ...
      if num_receive_antennas == 1
          projection_matrix_noncompliant = eye(num_receive_antennas);  % no nulling in SISO
      end
      ...
      if num_receive_antennas == 1 && num_transmit_antennas == 1
        % SISO case: use element-wise division to avoid ill-conditioned matrix warnings
        symbols_estimate_jammerless = squeeze(y_jammerless_frequency_tones(:,tone_index,:)) ./ (sqrt(num_subcarriers) * squeeze(user_channel_frequency_tones(:,:,tone_index)));
        symbols_estimate_jammerless = reshape(symbols_estimate_jammerless, [num_receive_antennas, num_transmit_antennas, num_ofdm_symbols]);

        symbols_estimate_compliant = projection_matrix_compliant * squeeze(y_compliant_frequency_tones(:,tone_index,:)) ./ (sqrt(num_subcarriers) * projection_matrix_compliant * squeeze(user_channel_frequency_tones(:,:,tone_index)));
        symbols_estimate_compliant = reshape(symbols_estimate_compliant, [num_receive_antennas, num_transmit_antennas, num_ofdm_symbols]);

        symbols_estimate_noncompliant = projection_matrix_noncompliant * squeeze(y_noncompliant_frequency_tones(:,tone_index,:)) ./ (sqrt(num_subcarriers) * projection_matrix_noncompliant * squeeze(user_channel_frequency_tones(:,:,tone_index)));
        symbols_estimate_noncompliant = reshape(symbols_estimate_noncompliant, [num_receive_antennas, num_transmit_antennas, num_ofdm_symbols]);
        ...
        for b=1:num_channel_taps
          symbols_estimate_projections(b,:,:) = squeeze(projectors(b,:,:)) * squeeze(y_noncompliant_frequency_tones(:,tone_index,:)) ./ (sqrt(num_subcarriers) * squeeze(projectors(b,:,:)) * squeeze(user_channel_frequency_tones(:,:,tone_index)));
        end
      else
        % MIMO case: use pseudo-inverse
        symbols_estimate_jammerless = pinv(sqrt(num_subcarriers)*(user_channel_frequency_tones(:,:,tone_index))) * squeeze(y_jammerless_frequency_tones(:,tone_index,:));  % zero-forcing estimate jammerless

        % Mitigated TVWS operator symbol estimation using projection matrix to suppress compliant jammer interference
        symbols_estimate_compliant = pinv(sqrt(num_subcarriers)*(projection_matrix_compliant*squeeze(user_channel_frequency_tones(:,:,tone_index)))) * (projection_matrix_compliant*squeeze(y_compliant_frequency_tones(:,tone_index,:)));  % zero-forcing with jammer mitigation compliant
        symbols_estimate_noncompliant = pinv(sqrt(num_subcarriers)*(projection_matrix_noncompliant*squeeze(user_channel_frequency_tones(:,:,tone_index)))) * (projection_matrix_noncompliant*squeeze(y_noncompliant_frequency_tones(:,tone_index,:)));  % zero-forcing with jammer mitigation noncompliant
        symbols_estimate_projections = zeros(num_receive_antennas,num_transmit_antennas,num_ofdm_symbols);  % initialize estimates for multiple projectors
        for b=1:num_channel_taps
          symbols_estimate_projections(b,:,:) = pinv(sqrt(num_subcarriers)*(squeeze(projectors(b,:,:))*squeeze(user_channel_frequency_tones(:,:,tone_index)))) * (squeeze(projectors(b,:,:))*squeeze(y_noncompliant_frequency_tones(:,tone_index,:)));  % zero-forcing with multiple nulling projectors
        end
      end
...
% Compute and print success rate of estimation and detection
% Success rate: percentage of SNR points where mitigated BER (noncompliant) is lower than unmitigated (compliant)
% Formula derivation:
% - results.BER_compliant and results.BER_noncompliant are vectors (one value per SNR point).
% - results.BER_compliant > results.BER_noncompliant: logical vector (1 if mitigation improved BER, 0 otherwise).
% - mean() gives fraction of SNR points where BER_noncompliant < BER_compliant (successful mitigation).
% - Multiply by 100 for percentage.
success_rate = mean(results.BER_compliant > results.BER_noncompliant) * 100;
fprintf('Success rate of jammer detection and mitigation: %.2f%%\n', success_rate);

% Print average BER for reference
fprintf('Average BER - Jammerless: %.4f\n', mean(results.BER_jammerless));
fprintf('Average BER - Compliant Jammer (unmitigated): %.4f\n', mean(results.BER_compliant));
fprintf('Average BER - Noncompliant Jammer (mitigated): %.4f\n', mean(results.BER_noncompliant));
